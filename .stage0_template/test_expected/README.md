...
## Publishing (optional GitHub Packages)

`make publish-package` / **`npm run publish-package`** pushes a semver package to **`npm.pkg.github.com`** for teams that want registry installs. Default SPA templates **consume via git** instead so CI does not require package-level **Manage Actions access**.
...
- **Consumers of the npm package:** each repo still needs **Package settings → Manage Actions access** (or a PAT) to use **`GITHUB_TOKEN`** against GPR—this is **not** automatable via a supported API; prefer **git dependencies** in templates to avoid that on the free tier.

## Usage

For complete working examples, see the [demo app](./demo/) which includes a **sign-in gate**, **navigation drawer** (hamburger), **component demo page**, and **admin (config) page**. The dev server can proxy `/api` to a local api_utils demo; auth uses `bootstrapDevAuthFromUrl` and localStorage, not `/dev-login`.

### Developer Edition: bootstrap from URL

Product SPAs should call **`bootstrapDevAuthFromUrl()` once before the router mounts** (e.g. from `src/initAuth.ts` imported from `main.ts`). It reads `#access_token=...&expires_at=...&roles=...` into the same `localStorage` keys as your auth composable, and handles `?dev_logout=1` for welcome-page logout. See umbrella [SPA standards](https://github.com/agile-learning-institute/mentorhub/blob/main/DeveloperEdition/standards/spa_standards.md) and `VITE_IDP_LOGIN_URI` for unauthenticated redirects.
