...
The package is **public** on [GitHub Packages](https://github.com/features/packages) (`npm.pkg.github.com`), **not** on registry.npmjs.org. GitHub’s npm API still requires a token to **install**, even when the package is public.
...
Configure **`//npm.pkg.github.com/:_authToken`** (PAT with **`read:packages`** or `gh auth token`) for local installs.

For **GitHub Actions** in another repo (e.g. an SPA Dockerfile running `npm install`), the workflow’s **`GITHUB_TOKEN`** can work **without a PAT secret** if you grant that repository access on **this package**: GitHub → **Packages** → this npm package → **Package settings** → **Manage Actions access** → add the SPA repo with at least **Read**.
...
- **Why auth exists:** **Publish** needs **`write:packages`**. **Install** from `npm.pkg.github.com` also needs a token (including for *public* packages)—unlike public packages on npmjs.org.
